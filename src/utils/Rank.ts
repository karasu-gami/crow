...
export async function addXpOnMessage(
  message: Message,
  xpAmount: number = 15
): Promise<boolean> {
  ...
  if (profile.rank) {
    ...
    if (profile.rank.level > oldLevel) {
      const embed = new EmbedBuilder()
        .setTitle("🎉 LEVEL UP!")
        .setDescription(
          `**${message.author.username}** subiu para **Nível ${profile.rank.level}**!`
        )
        .addFields(
          { name: "💰 EXP Ganho", value: `+${xpAmount} XP`, inline: true },
          {
            name: "📊 Próximo Nível",
...
async function levelUp(profile: any) {
